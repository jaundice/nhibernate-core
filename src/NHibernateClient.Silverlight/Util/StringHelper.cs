        ...
        public static string Replace(string template, string placeholder, string replacement, bool wholeWords)
        {
            ...
            else
            {
                ...
                if (loc + placeholder.Length < template.Length)
                {
                    ...
                    if (wholeWords &&
                        !(WhiteSpace.Contains(afterPlaceholder) || ClosedParen.Equals(afterPlaceholder) ||
                          Comma.Equals(afterPlaceholder)))
                    {
        ...
        public static string[] ParseFilterParameterName(string filterParameterName)
        {
            ...
            if (dot <= 0)
            {
                throw new ArgumentException(
                    "Invalid filter-parameter name format; the name should be a property path.", "filterParameterName");
            }
